...
export function SyllableSortExercise({ exercise, onSubmit }: ExerciseProps) {
  const prompt = exercise.content?.prompt ?? "Sort words into syllable types.";
  ...
  function submit() {
    ...
    for (const w of words) {
      if ((placed[w] ?? "") === (answer[w] ?? "")) correct += 1;
    }
    onSubmit(correct / words.length);
  }

  return (
    <ExerciseShell title="Syllable sort" prompt={prompt}>
